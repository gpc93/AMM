...
public class Filter extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (command != null) 
        {
            // Verifica che command sia uguale a search e sia stato passato q
            if (command.equals("search") && request.getParameter("q")!= null) 
            {    
                
                // Esegue la ricerca
                ArrayList<Prodotto> prodotti = HomeMadeFactory.getInstance().getProdottoByText(request.getParameter("q"));
                ...
                request.getRequestDispatcher("clientTableJson.jsp").forward(request, response);
            }
        }
